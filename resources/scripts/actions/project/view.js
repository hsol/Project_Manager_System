(function (root) {
    ...
    var listURL = "/project/list.asp?" + api.convert.objectToParameter(param);

    param.idx = location.href.getValueByKey("idx") ? location.href.getValueByKey("idx") : null
    var editURL = "/project/?" + api.convert.objectToParameter(param);
    ...
    api.ajax({ type: "GET", url: URL, data: param, contentType: "application/json;",
        success: function (data) {
            ...
            if (responseData.state) {
                var innerHTML = projectView.innerHTML.replace(RegExp("template", "gi"), "");

                projectView.innerHTML = null;
                projectView.innerHTML += convertTemplate.from(innerHTML, responseData);

                var selected = projectView.querySelector("#users ul.selected");
                if (responseData.users != null) {
                    if(responseData.users.length > 0) {
                        ...
                        api.remove.el(projectView.querySelector(".user"));
                    }
                }else{
                    api.remove.el(projectView.querySelector(".user"));
                }
                ...
                if (responseData.files != null) {
                    if(responseData.files.length > 0) {
                        var fileList = projectView.querySelector("#files ul");
                        ...
                        api.remove.el(projectView.querySelector(".file"));
                    }
                }else{
                    api.remove.el(projectView.querySelector(".file"));
                }
            ...
            success: function (data) {
                ...
                if (responseData.state) {
                    api.get.html("/templates/reply_default.html", function (html) {
                        ...
                        api.set.flexHeight(projectView);
                    });
                }
                else {
                    api.remove.el(document.querySelector("#replyList .moreReply"));
